...
function ReadBin(data, fType) {
  console.log("ReadBin size:", data.byteLength);
  console.log("Initial chunk size:", chunkSz);
  console.log("ReadBin, size:", data.byteLength);
  var s_action_chunk = Module._malloc(128);
  ...
  if (chunkSz === 0) {
    console.warn("No chunks to read, file may not be parsed");
    return;
  }
  ...
  for (;;) {
    ...
    console.log("Next chunk size:", chunkSz);
    Module._free(res_ptr);
    if (chunkSz === 0) {
      break;
    }
  ...
  console.log("Calling post-processing");
  post_proc_file_cb(fType, 0);
}
...
function loadFile(filePath) {
  ...
  req.onload = (event) => {
    ...
    if (arrayBuffer) {
      ...
      console.log("Fetched LAS size:", byteArray.length); // <-- log size
      ReadBin(byteArray, 0);
    }
    if (req.readyState == 4) {
      const post_proc_file_cb = GetPostProcCB();
      post_proc_file_cb(0, 0);
      console.log("Post-processing complete"); // <-- log post-processing
    }
  };
  req.onerror = (e) => {
...
// function loadFile(filePath) {
//   const req = new XMLHttpRequest();
//   req.open("GET", filePath, true);   // use the argument, not a hard-coded URL
//   req.responseType = "arraybuffer";
//   req.onload = (event) => {
//     const arrayBuffer = req.response;
//     if (arrayBuffer) {
//       const byteArray = new Uint8Array(arrayBuffer); 
//       ReadBin(byteArray, 0); // fType=0 for LAS
//     }
//     if (req.readyState == 4) {
//       post_proc_file_cb = GetPostProcCB();
//       post_proc_file_cb(0, 0);
